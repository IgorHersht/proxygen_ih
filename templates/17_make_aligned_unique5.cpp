...
template <typename T, std::align_val_t al, typename ... Args> auto make(Args&& ... args){
    static_assert(size_t(al) >= sizeof(void *), " small size");
    static_assert(isPowerOf2(size_t(al)), " bad size");
    static_assert((size_t(al)%size_t(alignof(T)) == 0), " bad size");
    AlignDeleter<T, al> del;
    Ptr<T, al> ptr(new (al) T(std::forward<Args>(args) ...), del);
    return ptr;
}
