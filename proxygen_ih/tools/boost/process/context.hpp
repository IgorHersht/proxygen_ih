 ...
 */
template<class Path>
class basic_context: public basic_work_directory_context<Path>,
...
public:
	 ...
	 * values. 
	 */
	basic_pipeline_entry(const Executable &exe, const Arguments &args,
			const Context &ctx) :
			executable(exe), arguments(args), context(ctx) {
	}
};
