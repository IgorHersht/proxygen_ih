...
public:
	...
	posix_child(id_type id, detail::info_map &infoin, detail::info_map &infoout) :
			child(id,
					...
					detail::posix_info_locate_pipe(infoout, STDOUT_FILENO,
							true),
					detail::posix_info_locate_pipe(infoout, STDERR_FILENO,
							true)) {
		for (detail::info_map::iterator it = infoin.begin(); it != infoin.end();
				++it) {
			detail::stream_info &si = it->second;
			if (si.type_ == detail::stream_info::use_pipe) {
				BOOST_ASSERT(si.pipe_->wend().valid());
				boost::shared_ptr < postream
						> st(new postream(si.pipe_->wend()));
		...
		for (detail::info_map::iterator it = infoout.begin();
			...
			if (si.type_ == detail::stream_info::use_pipe) {
				...
				boost::shared_ptr < pistream
						> st(new pistream(si.pipe_->rend()));
