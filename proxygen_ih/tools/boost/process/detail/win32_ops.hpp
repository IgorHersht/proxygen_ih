...
inline boost::shared_array<char> collection_to_win32_cmdline(
 ...
 */
inline boost::shared_array<char> environment_to_win32_strings(
...
inline PROCESS_INFORMATION win32_start(const Executable &exe,
		const Arguments &args, const environment &env, stream_info &infoin,
		stream_info &infoout, stream_info &infoerr, const win32_setup &setup) {
	...
	case stream_info::use_file: {
		HANDLE h = ::CreateFileA(infoin.file_.c_str(), GENERIC_READ, 0, NULL,
	...
	case stream_info::use_file: {
		HANDLE h = ::CreateFileA(infoout.file_.c_str(), GENERIC_WRITE, 0, NULL,
	...
	case stream_info::use_file: {
		HANDLE h = ::CreateFileA(infoerr.file_.c_str(), GENERIC_WRITE, 0, NULL,
				CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (h == INVALID_HANDLE_VALUE)
			boost::throw_exception(
					boost::system::system_error(
	...
	boost::scoped_array<char> workdir(
			new char[setup.work_directory.size() + 1]);
#if defined(__CYGWIN__) 
	::strcpy(workdir.get(), setup.work_directory.c_str());
#else 
	::strcpy_s(workdir.get(), setup.work_directory.size() + 1,
			setup.work_directory.c_str());
#endif 

	boost::shared_array<char> envstrs = environment_to_win32_strings(env);

	if (!::CreateProcessA(executable.get(), cmdline.get(), NULL, NULL, TRUE, 0,
			envstrs.get(), workdir.get(), si.get(), &pi))
		boost::throw_exception(
				boost::system::system_error(
						boost::system::error_code(::GetLastError(),
								boost::system::get_system_category()),
