...
void RequestHandlerAdaptor::setHTTPServerAcceptor(HTTPServerAcceptor* acceptor)
		noexcept {
	m_acceptor = CHECK_NOTNULL(acceptor);

}

RequestHandlerAdaptor::~RequestHandlerAdaptor() {
...
void RequestHandlerAdaptor::onHeadersComplete(std::unique_ptr<HTTPMessage> msg)
	...
	if (msg->getHeaders().exists(HTTP_HEADER_EXPECT)) {
		auto expectation = msg->getHeaders().getSingleOrEmpty(
				HTTP_HEADER_EXPECT);
		if (!boost::iequals(expectation, "100-continue")) {
			setError(kErrorUnsupportedExpectation);

			ResponseBuilder(this).status(417, "Expectation Failed").closeConnection().sendWithEOM();
		} else {
