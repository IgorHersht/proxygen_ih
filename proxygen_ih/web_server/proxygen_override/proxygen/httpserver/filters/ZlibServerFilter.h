...
public:
	explicit ZlibServerFilter(RequestHandler* downstream,
			int32_t compressionLevel, uint32_t minimumCompressionSize,
			const std::shared_ptr<std::set<std::string>> compressibleContentTypes) :
			Filter(downstream), compressionLevel_(compressionLevel), minimumCompressionSize_(
					minimumCompressionSize), compressibleContentTypes_(
...
public:
	explicit ZlibServerFilterFactory(int32_t compressionLevel,
			uint32_t minimumCompressionSize,
			const std::set<std::string> compressibleContentTypes) :
			compressionLevel_(compressionLevel), minimumCompressionSize_(
					minimumCompressionSize), compressibleContentTypes_(
	...
	RequestHandler* onRequest(RequestHandler* h, HTTPMessage* msg)
		...
		if (acceptsSupportedCompressionType(msg)) {
			auto zlibServerFilter = new ZlibServerFilter(h, compressionLevel_,
					minimumCompressionSize_, compressibleContentTypes_);
...
protected:
	...
	bool acceptsSupportedCompressionType(HTTPMessage* msg) noexcept {
		...
		auto acceptEncodingHeader = msg->getHeaders().getSingleOrEmpty(
				HTTP_HEADER_ACCEPT_ENCODING);

		if (RFC2616::parseQvalues(acceptEncodingHeader, output)) {
			std::vector<RFC2616::TokenQPair>::iterator it =
					std::find_if(output.begin(), output.end(),
							[](RFC2616::TokenQPair elem) {
