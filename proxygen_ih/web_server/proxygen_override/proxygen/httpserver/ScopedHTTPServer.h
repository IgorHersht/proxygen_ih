...
public:
	explicit ScopedHandler(HandlerType* ptr) :
			handlerPtr_(ptr) {
	}
	...
	void onEOM() noexcept override {
		try {
			...
			ResponseBuilder(downstream_).status(500, "Internal Server Error").body(
					ex.what()).sendWithEOM();
		} catch (...) {
			ResponseBuilder(downstream_).status(500, "Internal Server Error").body(
					"Unknown exception thrown").sendWithEOM();
		}
	}
...
public:
	explicit ScopedHandlerFactory(HandlerType handler) :
			handler_(handler) {
	}
...
inline std::unique_ptr<ScopedHTTPServer> ScopedHTTPServer::start(
		HandlerType handler, int port, int numThreads,
		std::unique_ptr<wangle::SSLContextConfig> sslCfg) {

	std::unique_ptr<RequestHandlerFactory> f = folly::make_unique<
...
inline std::unique_ptr<ScopedHTTPServer> ScopedHTTPServer::start<
		...
		std::unique_ptr<RequestHandlerFactory> f, int port, int numThreads,
		std::unique_ptr<wangle::SSLContextConfig> sslCfg) {
