...
public:
	AcceptorFactory(std::shared_ptr<HTTPServerOptions> options,
			...
			options_(options), codecFactory_(codecFactory), config_(config), sessionInfoCb_(
					sessionInfoCb) {
	}
	std::shared_ptr<wangle::Acceptor> newAcceptor(folly::EventBase* eventBase)
			override {
		auto acc =
				std::shared_ptr<HTTPServerAcceptor>(
						HTTPServerAcceptor::make(config_, *options_,
								codecFactory_).release());
		if (sessionInfoCb_) {
...
HTTPServer::HTTPServer(HTTPServerOptions options) :
	...
	if (!options_->supportsConnect) {
		options_->handlerFactories.insert(options_->handlerFactories.begin(),
...
void HTTPServer::start(std::function<void()> onSuccess,
	...
	try {
		FOR_EACH_RANGE (i, 0, addresses_.size())
		{
			...
			auto accConfig = HTTPServerAcceptor::makeConfig(addresses_[i],
					*options_);
			auto factory = std::make_shared<AcceptorFactory>(options_,
					codecFactory, accConfig, sessionInfoCb_);
	...
	if (onSuccess) {
		mainEventBase_->runInLoop([onSuccess]() {
...
static void stopAcceptor(wangle::Acceptor *acceptor) {
	HTTPServerAcceptor *httpServerAcceptor =
			dynamic_cast<HTTPServerAcceptor *>(acceptor);
	if (!httpServerAcceptor) {
		return;
	}
	folly::EventBase* eb = httpServerAcceptor->getEventBase();
	if (!eb) {
		return;
	}
	try {
		eb->runInEventBaseThreadAndWait([httpServerAcceptor]() {
			httpServerAcceptor->requestStop();
		});
	} catch (...) {
...
void HTTPServer::stop() {
	...
	for (auto& bootstrap : bootstrap_) {
		bootstrap.forEachWorker(stopAcceptor);
	}
...
const std::vector<const folly::AsyncSocketBase*> HTTPServer::getSockets() const {
...
int HTTPServer::getListenSocket() const {
	...
	auto serverSocket = std::dynamic_pointer_cast<folly::AsyncServerSocket>(
			bootstrapSockets[0]);
